...
const AI_SUGGESTIONS = [
  { text: "Boost fiber internet coverage in Barangay Poblacion to lift work readiness score.", tag: "Internet" },
  { text: "Partner with 2–3 long-stay guesthouses to expand accommodation options.", tag: "Accom." },
  { text: "Add a community transport schedule to improve access score.", tag: "Transport" },
  { text: "Register more work cafes with verified co-working amenities.", tag: "Work spots" },
  { text: "Apply for DOT accreditation to strengthen tourism credibility.", tag: "Tourism" },
];
    ...
    <div style={{ background: "#FDFBF8", border: "1px solid #F4EFE7", borderRadius: 12, padding: 20 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {AI_SUGGESTIONS.map((s, i) => {
          const accent = TAG_COLORS[s.tag] || { bg: "#f0ece6", color: "#888" };
          return (
            <div
              key={i}
            ...
            >
              ...
              >
                <p style={{ margin: 0, fontSize: 12, color: "#555", lineHeight: 1.5 }}>{s.text}</p>
                <span style={{
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.getCurrentScore()
      .then((data) => { if (mounted) { setSnapshot(data || null); setLoading(false); } })
      ...
      <div className="db-main">
        {loading ? (
          ...
          <>
            ...
            {/* Row 2: ScoreCard (left) | AI Suggestions + Overall Score (right) */}
            <div ref={gridRef} style={{ opacity: 0, transform: "translateY(16px)" }}>
              <div className="db-grid">
                {/* Left: ScoreCard — hideOverallScore suppresses its built-in summary card */}
